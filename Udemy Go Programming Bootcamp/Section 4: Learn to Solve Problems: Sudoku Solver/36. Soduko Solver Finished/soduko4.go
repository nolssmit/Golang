...
// Receives slice, number to check, row & column
// and decides if that number fits the rules of the game
func isNumValid(puzz [][]int, guess int, row int, column int) bool {

	// Cycle through all values in row to see
	// if the row is valid (We only want index which is the 1st value)
	for index := range puzz {
		// Use index for cycling
		// Check if any of the values are equal to our guess and
		// also that we didn't already place it there in this function
	...
	// into their 3x3 version so we can cycle through them
	...
	for k := 0; k < 3; k++ {
		for l := 0; l < 3; l++ {
			// We need to check if any element in the box is equal to what we just
			// added but skip checking the position we just placed our value in
...
// Recursively solves puzzle by placing values if it reaches a point
// where there is no valid answer it changes the previous supposedly correct
// answer back and tries another value for what we previously though was correct
...
func solvePuzzle(puzz [][]int) bool {
	// Check for empty space and if there is none return
	// true meaning the puzzle is solved, otherwise solve the puzzle
	...
	// Check if we can add a 1 through 9 into the empty space
	for i := 1; i <= 9; i++ {
		...
		if isNumValid(puzz, i, row, column) {
			...
			// Continue until solvePuzzle with new puzz until it returns
			// true meaning we no longer have empty spaces and we are finished
			if solvePuzzle(puzz) {
				return true
			}
			// If solvePuzzle doesn't return true we reset to an empty
			// space and backtrack and try the next value in the for loop
			puzz[row][column] = 0
		}
	}
	return false
