...
func isNumValid(puzz [][]int, guess int, row int, column int) bool {
	...
	// Cycle through all values in row to see if the row 
	// is valid (We only want index which is the 1st value)
...
func main() {
	...
	// Testing if a guess is valid (starting to count rows and columns from zero)
	// The parameters are: <guess>, <row>, <column>
	fmt.Println("--------- Is 7 valid for row 4, col 0 ---------")
	fmt.Println(isNumValid(puzz, 7, 4, 0))
	fmt.Println("--------- Is 1 valid for row 1, col 7 ---------")
